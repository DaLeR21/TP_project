...
void Print::units(Player* player) {
  for (int i = 0; i < player->units.size(); i++) {
    cout << "\t"
         << "(" << i + 1 << ") " << game.idtoname(player->units[i].id) << " "
         << player->units[i].dmg << " " << player->units[i].hp << " ";
    if (not player->units[i].canAttack) cout << "-";
    cout << "\n";
  }
}

void Print::info(Player* player) {
  cout << "\n" << player->name << "Золото: " << (player->gold) << "\n";
  for (int i = 0; i < cmd.size(); i++) cout << cmd[i].name;
}
...
void Print::all_units() {
  ...
  for (int i = 0; i < unit_ids.size(); i++)
    if (i != 3)
      cout << "\t"
           << " (" << idtounit(i + 1, unit).id << ") " << game.idtoname(i + 1)
           << " " << idtounit(i + 1, unit).dmg << " "
           << idtounit(i + 1, unit).hp
           << " Стоимость: " << idtounit(i + 1, unit).price << "\n";
}
...
void Game::create_unit_from_user(Player* player, AbstractFactory& factory) {
  ...
  do {
    ...
    do {
      cin >> tt;
      if ((tt < 1) or (tt > unit_ids.size())) cout << "Неправильное число\n";
...
void Game::set_attack(Player* player) {
  for (int i = 0; i < player->units.size(); i++) player->units[i].canAttack = 1;
}
...
void Game::do_attack(Player* player, Player* opp) {
  do {
    ...
    cout << opp->name;
    print.units(opp);
    cout << "\t(" << player->units.size() + 1 << ") Выход\n";
    bool b;
    do {
      b = false;
      cin >> t1;
    ...
    if (t1 == player->units.size() + 1) break;
    ...
    player->units[t1 - 1].hp -= opp->units[t2 - 1].dmg *
                                player->units[t1 - 1].takendmg *
                                opp->units[t2 - 1].attackdmg;
